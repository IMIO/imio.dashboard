...
class TestCombinedIndex(IntegrationTestCase):
    ...
    def setUp(self):
        ...
        self.folder1 = api.content.create(
            id="folder1", type="Folder", title="Folder 1", container=self.portal
        )
        ...
        self.folder3 = api.content.create(
            id="folder3", type="Folder", title="Folder 3", container=self.portal
        )
        ...
        self.assertEquals(
            ...
            ["Document", "Document__private", "Folder", "Folder__private", "private"],
        )
    ...
    def test_combined_index(self):
        ...
        self.assertEquals(
            faceted_query.criteria()["contained_types_and_states"]["query"], "private"
        )
        ...
        self.assertEquals(
            faceted_query.criteria()["contained_types_and_states"]["query"], "Document"
        )
        # we get folder1 and folder3
        uids = [brain.UID for brain in faceted_query.query()]
        self.assertEquals(len(faceted_query.query()), 2)
    ...
    def test_catalog_indexes_vocabulary(self):
        ...
        vocab = queryUtility(
            IVocabularyFactory, "eea.faceted.vocabularies.CatalogIndexes"
        )(self.portal)
        ...
        real_indexes = [
            token
            for token in tokens
            if token and not token.startswith(COMBINED_INDEX_PREFIX)
        ]
        combined_indexes = [
            token
            for token in tokens
            if token and token.startswith(COMBINED_INDEX_PREFIX)
        ]
