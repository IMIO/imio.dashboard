...
class IDRenderCategoryView(RenderCategoryView):
    ...
    def contact_infos(self):
        return {
            "orgs-searches": {
                ...
                "img": "organization_icon.png",
            },
            "hps-searches": {
                "typ": "contact",
                "add": "@@add-contact",
                "img": "create_contact.png",
            },
            "persons-searches": {
                "typ": "person",
                "add": "++add++person",
    ...
    def _get_category_template(self):
        if self.manage_add_contact_actions and IContactsDashboard.providedBy(
            self.context
        ):
...
class CombinedFacetedQueryHandler(FacetedQueryHandler):
    def criteria(self, sort=False, **kwargs):
        ...
        criteria = super(CombinedFacetedQueryHandler, self).criteria(
            sort=sort, **kwargs
        )
        res = criteria.copy()
        for key, value in criteria.items():
            if key == "facet.field":
                res[key] = [e for e in value if not e.startswith(COMBINED_INDEX_PREFIX)]
